...
> **日期**: 2026-07-18 | **模型**: YOLOv11n 1920×1920 4类检测 | **GPU**: NVIDIA5090 | **测试视频**: 16499 帧 1080p
...
| **C++ batch=4 + 异步解码** 🚀 | 1318           | 0.76 ms           | **643 fps**  | **25.7s** | **8.5×**  |

> **管线 FPS** = GPU 纯计算速度（不含 I/O 等待）。**真实吞吐** = 含视频解码 + I/O 的端到端速度。
>
> **各行含义**：
>
> | 行 | 说明 |
> |------|------|
> | Python batch=1 | `model(frame)` 单帧推理，包含 PyTorch GPU 推理 + CPU NMS |
> | C++ 原始（CPU NMS） | C++ TensorRT 推理（GPU），但后处理全在 CPU：逐行遍历 8400 个检测框 + `dnn::NMSBoxes` |
> | C++ GPU 解码（单 Stream） | 新增 `postprocess.cu` CUDA kernel，8400 线程并行做置信度过滤 + 坐标解码，数据传输 2.7MB→1.2KB；NMS 仍在 CPU，但只有几十个框 |
> | C++ 双 Stream batch=1 | 2 个 ExecutionContext + 2 CUDA Stream，GPU 处理下一帧时 CPU 做当前帧 NMS，时间重叠 |
> | C++ 双 Stream batch=2/4/8 | 引擎内 batch=N，一次推理 N 帧。batch=4 为甜点：GPU 利用率高 + 显存合理。batch=8 因 pre/post 开销反超推理收益而下降 |
> | C++ batch=4 + 异步解码 | FrameQueue + 独立解码线程，`cv::VideoCapture::read()` 在后台提前读帧，GPU 不等磁盘 I/O |
>
> **注意**："GPU 解码"指 GPU 做置信度过滤+bbox 解码，**不是** NMS 在 GPU 上（NMS 仍用 CPU 的 `dnn::NMSBoxes`）。真正的 GPU NMS 见 `feat/efficient-nms` 分支。
>
> **术语备注**：
>
> | 术语 | 解释 |
...
> 异步解码（batch=4）：管道 1318 FPS，**真实 643 FPS，25.7s**。
